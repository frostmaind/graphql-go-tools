...
func (d *dataLoader) Load(ctx *Context, fetch *SingleFetch, responsePair *BufPair) (err error) {
	...
	fetchParams, err := d.selectedDataForFetch(parentResult.data(), fetch.OnType, ctx.responseElements...)
	if err != nil {
		return err
	}
...
func (d *dataLoader) LoadBatch(ctx *Context, batchFetch *BatchFetch, responsePair *BufPair) (err error) {
	...
	fetchParams, err := d.selectedDataForFetch(parentResult.data(), batchFetch.Fetch.OnType, ctx.responseElements...)
	if err != nil {
		return err
	}
